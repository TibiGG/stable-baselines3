...
class MultiAgentMultiInputPolicy(BasePolicy):
    """
    Policy class (with both actor and critic) for TD3.
    ...
    :param activation_fn: Activation function
    :param features_extractor_class: Features extractor to use.
            # TODO: Create distinction between actor and critic features_extractor
            #    features_extractor_class: Type[BaseFeaturesExtractor] = MultiAgentCombinedExtractor,
    :param features_extractor_kwargs: Keyword arguments
        to pass to the features extractor.
    :param normalize_images: Whether to normalize images or not,
         dividing by 255.0 (True by default)
    ...
    def __init__(
            ...
            lr_schedule: Schedule,
            net_arch: Optional[
                Union[List[int], Dict[str, List[int]]]] = None,
            activation_fn: Type[nn.Module] = nn.ReLU,
            actor_features_extractor_class: Type[
                BaseFeaturesExtractor] = CombinedExtractor,
            actor_features_extractor_kwargs: Optional[Dict[str, Any]] = None,
            critic_features_extractor_class: Type[
                BaseFeaturesExtractor] = TupleCombinedExtractor,
        ...
        super().__init__(
            observation_space,
            action_space,
            features_extractor_class=actor_features_extractor_class,
            features_extractor_kwargs=actor_features_extractor_kwargs,
            optimizer_class=optimizer_class,
            optimizer_kwargs=optimizer_kwargs,
            squash_output=True,
        )
        ...
        # Default network architecture, from the original paper
        if net_arch is None:
            if actor_features_extractor_class == NatureCNN:
                net_arch = []
            else:
                net_arch = [400, 300]
        ...
        self.activation_fn = activation_fn
        self.net_args = {
            "observation_space": self.observation_space.spaces[0],
            "action_space": self.action_space.spaces[0],
            "net_arch": actor_arch,
            "activation_fn": self.activation_fn,
            "normalize_images": normalize_images,
        }
    ...
    def make_actor(self, features_extractor: Optional[
        BaseFeaturesExtractor] = None, ) -> Actor:
        actor_kwargs = self._update_features_extractor(self.actor_kwargs,
                                                       features_extractor)
        return Actor(**actor_kwargs).to(self.device)

    def make_critic(self, features_extractor: Optional[
        BaseFeaturesExtractor] = None) -> ContinuousCritic:
        critic_kwargs = self._update_features_extractor(self.critic_kwargs,
                                                        features_extractor,
                                                        self.critic_features_extractor_class,
